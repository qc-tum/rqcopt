...
def create_ising_hamiltonian():
    ...
    return qib.IsingHamiltonian(field, J, 0., g)


def ising1d_blockenc_opt(nlayers: int, bootstrap: bool, real: bool, rng: np.random.Generator = None, anc = None, **kwargs):
    """
    ...
    if bootstrap:
        ...
        with h5py.File(f"ising1d_blockenc_opt_n{nlayers-2}_{oum}.hdf5", "r") as f:
            ...
            assert f.attrs["L"] == L
            assert f.attrs["J"] == H_op.J
            assert f.attrs["g"] == H_op.g
            if anc is not None:
    ...
    plt.title(f"optimization progress for a quantum circuit with {len(Vlist)} layers (Ising)")
    plt.show()
    # target function
    plt.semilogy(range(len(f_iter)), np.array(f_iter))
    plt.xlabel("iteration")
    plt.ylabel(r"$f(\mathrm{Vlist})$")
    plt.title(f"optimization target function for a quantum circuit with {len(Vlist)} layers (Ising)")
    plt.show()
    ...
    with h5py.File(f"ising1d_blockenc_opt_n{nlayers}_{oum}.hdf5", "w") as f:
        ...
        # store parameters
        f.attrs["L"] = L
        f.attrs["J"] = float(H_op.J)
        f.attrs["g"] = float(H_op.g)
        f.attrs["prob"] = prob
        if anc is not None:
            f.attrs["anc"] = anc
...
def main():
    ...
    # 3 layers
    ising1d_blockenc_opt(3, False, real, rng, anc, niter=100)

    # 5 layers
    ising1d_blockenc_opt(5, restart, real, rng, anc, niter=100)

    # 7 layers
    ising1d_blockenc_opt(7, restart, real, rng, anc, niter=100)
    
    # 9 layers
    ising1d_blockenc_opt(9, restart, real, rng, anc, niter=100)
