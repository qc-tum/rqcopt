...
def create_fermi_hubbard_hamiltonian():
    ...
    return qib.operator.FermiHubbardHamiltonian(field, float(t), float(u), spin)


def fermihubbard_blockenc_opt(nlayers: int, bootstrap: bool, real: bool, rng: np.random.Generator = None, anc = None, **kwargs):
    """
    Optimize the quantum gates in a brickwall layout to approximate
    a Fermi-Hubbard Hamiltonian by block-encoding.
    """
    ...
    if bootstrap:
        ...
        with h5py.File(f"fermihubbard_blockenc_opt_n{nlayers-2}_{oum}.hdf5", "r") as f:
            # parameters must agree
            assert f.attrs["L"] == L
            assert f.attrs["t"] == H_op.t
            assert f.attrs["u"] == H_op.u
            assert f.attrs["spin"] == H_op.spin
            if anc is not None:
    ...
    plt.title(f"optimization progress for a quantum circuit with {len(Vlist)} layers (fermihubbard)")
    plt.show()
    # target function
    plt.semilogy(range(len(f_iter)), np.array(f_iter))
    plt.xlabel("iteration")
    plt.ylabel(r"$f(\mathrm{Vlist})$")
    plt.title(f"optimization target function for a quantum circuit with {len(Vlist)} layers (fermihubbard)")
    plt.show()
    ...
    oum = "real" if real else "complex"
    with h5py.File(f"fermihubbard_blockenc_opt_n{nlayers}_{oum}.hdf5", "w") as f:
        f.create_dataset("Vlist", data=Vlist)
        f.create_dataset("f_iter", data=f_iter)
        f.create_dataset("err_iter", data=err_iter)
        # store parameters
        f.attrs["L"] = L
        f.attrs["t"] = float(H_op.t)
        f.attrs["u"] = float(H_op.u)
        f.attrs["spin"] = H_op.spin
        f.attrs["prob"] = prob
        if anc is not None:
            f.attrs["anc"] = anc
...
def main():
    ...
    # 3 layers
    fermihubbard_blockenc_opt(3, False, real, rng, anc, niter=100)

    # 5 layers
    fermihubbard_blockenc_opt(5, restart, real, rng, anc, niter=100)

    # 7 layers
    fermihubbard_blockenc_opt(7, restart, real, rng, anc, niter=100)
