...
def compute_circuit_errors(J, g, Llist, t, nlayers):
    ...
    for j, n in enumerate(nlayers):
        with h5py.File(f"ising1d_disordered_dynamics_opt_n{n}.hdf5", "r") as f:
...
def main(recompute=True):
    ...
    if recompute:
        ...
        # save errors to disk
        with h5py.File("ising1d_disordered_dynamics_approx_larger_systems.hdf5", "w") as f:
        ...
        with h5py.File("ising1d_disordered_dynamics_approx_larger_systems.hdf5", "r") as f:
            # parameters must agree
            assert f.attrs["J"] == J
            assert f.attrs["g"] == g
            assert f.attrs["t"] == t
    ...
    plt.savefig("ising1d_disordered_dynamics_approx_larger_systems.pdf")
    plt.show()
