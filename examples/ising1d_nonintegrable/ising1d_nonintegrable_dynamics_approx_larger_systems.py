...
def compute_circuit_errors(J: float, g: float, h: float, Llist, t: float, nlayers):
    """
    ...
    for j, n in enumerate(nlayers):
        with h5py.File(f"ising1d_nonintegrable_dynamics_opt_n{n}.hdf5", "r") as f:
...
def main(recompute=True):
    ...
    if recompute:
        circ_err = compute_circuit_errors(J, g, h, Llist, t, nlayers)
        # save errors to disk
        with h5py.File("ising1d_nonintegrable_dynamics_approx_larger_systems.hdf5", "w") as f:
        ...
        with h5py.File("ising1d_nonintegrable_dynamics_approx_larger_systems.hdf5", "r") as f:
            # parameters must agree
            assert f.attrs["J"] == J
            assert f.attrs["g"] == g
            assert f.attrs["h"] == h
    ...
    plt.title(rf"$\mathrm{{approximating }}\ e^{{-i H^{{\mathrm{{Ising}}}} t}} \ \mathrm{{for}} \ J = {J}, g = {g}, h = {h}, t = {t}$")
    plt.savefig("ising1d_nonintegrable_dynamics_approx_larger_systems.pdf")
    plt.show()
