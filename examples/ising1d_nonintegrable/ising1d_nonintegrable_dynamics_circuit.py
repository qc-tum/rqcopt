...
def construct_ising_local_term(J: float, g: float, h: float):
    """
    Construct local interaction term of Ising Hamiltonian on a one-dimensional
    lattice for interaction parameter `J` and external field parameters `g` and `h`.
    """
    # Pauli-X and Z matrices
    X = np.array([[0.,  1.], [1.,  0.]])
    Z = np.array([[1.,  0.], [0., -1.]])
    I = np.identity(2)
    return J*np.kron(Z, Z) + g*0.5*(np.kron(X, I) + np.kron(I, X)) + h*0.5*(np.kron(Z, I) + np.kron(I, Z))
...
def main():
    ...
    H = qib.IsingHamiltonian(field, J, h, g).as_matrix()
    ...
    plt.title(f"Ising Hamiltonian for J = {J}, g = {g}, h = {h} on a 1D lattice with {L} sites")
    plt.show()
    ...
    hloc = construct_ising_local_term(J, g, h)
    ...
    for nlayers in [3, 5, 7, 9]:
        with h5py.File(f"ising1d_nonintegrable_dynamics_opt_n{nlayers}.hdf5", "r") as f:
    ...
    plt.title(rf"$\mathrm{{approximating }}\ e^{{-i H^{{\mathrm{{Ising}}}} t}} \ \mathrm{{for}} \ L = {L}, J = {J}, g = {g}, h = {h}, t = {t}$")
    plt.savefig("ising1d_nonintegrable_dynamics_circuit_error.pdf")
    plt.show()
